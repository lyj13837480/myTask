...
describe("stake test", async function () {
    ...
    it("deploy", async function () {
        ...
        // 部署 MetaNodeStake
        const metaNodeStake = await ethers.getContractFactory("MyStake")
    ...
    it("setToken", async () => {
        ...
        await stakeProxyContract.connect(admin).setToken(erc20ddress)
        const newERC20 = await stakeProxyContract.token()
        expect(newERC20).to.eq(erc20ddress)
    })

    it("stopWithdraw", async () => {
        await stakeProxyContract.connect(admin).stopWithdraw()
        const res = await stakeProxyContract.withdrawEnabled()
        expect(res).to.true
    })

    it("startWithdraw", async () => {
        await stakeProxyContract.connect(admin).startWithdraw()
        const res = await stakeProxyContract.withdrawEnabled()
        expect(res).to.false
    })

    it("stopClaim", async () => {
        await stakeProxyContract.connect(admin).stopClaim()
        const res = await stakeProxyContract.claimEnabled()
        expect(res).to.true
    })

    it("startClaim", async () => {
        await stakeProxyContract.connect(admin).startClaim()
        const res = await stakeProxyContract.claimEnabled()
        expect(res).to.false
    })
    ...
    it("deposit", async () => {
        ...
        await stakeProxyContract.connect(user3).depositTokens(1, ethers.parseEther("200"))

      
        console.log("metaNodeStakeContract::", proxyAddress)
        const add = await erc20Contract.balanceOf(proxyAddress)
    ...
    it("withdraw", async () => {
        ...
        const metaNodeStakeAddress = await stakeProxyContract.getAddress()
        console.log("metaNodeStakeContract::", metaNodeStakeAddress)
        const add = await erc20Contract.balanceOf(metaNodeStakeAddress)
        ...
        console.log("stakeContract erc20 balance::", add)
